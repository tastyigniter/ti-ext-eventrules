...
class BaseEvent extends AbstractBase
{
    ...
    {
        $results = [];
        $eventRules = ExtensionManager::instance()->getRegistrationMethodValues('registerEventRules');
        if (is_null($key))
            return $eventRules;
    ...
    {
        $results = [];
        foreach (self::findEvents() as $eventClass => list($eventCode, $eventObj)) {
    ...
    {
        foreach (self::findEvents() as list($eventCode, $eventObj)) {
