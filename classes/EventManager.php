...
use App;
use Event;
use Igniter\EventRules\Jobs\ProcessEventRule;
...
class EventManager
{
    ...
    public static function bindEventRules()
    {
        foreach (BaseEvent::findEvents() as $eventClass => list($eventCode, $eventObj)) {
    ...
    public function queueEvent($eventClass, array $params)
    {
        ...
        Queue::push(new ProcessEventRule($eventClass, $params));
    }

    public function fireEvent($eventClass, array $params)
    {
        $models = EventRule::listRulesForEvent($eventClass);
